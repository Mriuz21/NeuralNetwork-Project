...
def TestModel(_X_test, _Y_test, version):
...
def submit_digit(image, label):
    # Append the new image and label to the lists
...
def train_model():
    ...
    X_new = X_new.reshape(-1, X_train.shape[1])
    Y_new = np.array(new_labels)

    save_dir = 'Models/V' + str(13)
    os.makedirs(f'Models', exist_ok=True)
    os.makedirs(save_dir, exist_ok=True)

    np.savez(save_dir + '/combined_data.npz', X=X_new, Y=Y_new)

    CreateModel(X_new, Y_new, _epochs=15, version=13)
    
    # Clear the new data lists
    new_images.clear()
    new_labels.clear()
...
if __name__ == '__main__':
  ...
  #CreateModel(X_train, Y_train, 32, 12)
  
  def predict_digit_callback(image, label):
    prediction = PredictDigit(image, version=13)
    gui.display_prediction(prediction)
    if gui.current_label != None:
       submit_digit(image, label)
  gui = DigitRecGUI(window,predict_digit_callback,train_model)
